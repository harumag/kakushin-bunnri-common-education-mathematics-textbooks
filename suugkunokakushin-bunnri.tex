...
\begin{center}
    \ans{14章の問題演習}
\end{center}
\reibanichi 次のような答案の意味不明な部分を指摘せよ.\\
\kakkoichi 問題：3点A$\p{1,1}$,B$\p{3,2}$,C$\p{9,c}$が一直線上にあるとき,$c$を求めよ.\\
\kai 3点A,B,Cは一直線上にあるので,$\Vec{AC}=k\Vec{AB}$である.これを成分表示すると,
\begin{align*}
    \tvec<0,1>[8,c-1]=k\tvec<0,1>[2,1]
\end{align*}
これを解くと,$k=4,c=5$より,答えは,$c=5$\\\\
\kakkoni 問題：$\sqrt{2}$が無理数であることを示せ.\\
\kai$\sqrt{2}$が有理数であると仮定すると,$\sqrt{2}=\dfrac{p}{q}（p,qは互いに素な整数）$\\
よって,$p=\sqrt{2}q\doti p^2=2q^2\cdots\cdots\maruichi$\\
$\maruichi の右辺=偶数より,\maruichi の左辺=偶数で,したがって,p=偶数$\\
よって,$p=2m（mは整数）$で,
\begin{align*}
    \maruichi\doti\p{2m}^2=2q^2\doti2m^2=q^2\cdots\cdots\maruni
\end{align*}
$\maruni の左辺=偶数より,\maruni の右辺=偶数で,したがって,q=偶数$\\
ところが,これでは,$pもqも$偶数になってしまい,$pとq$が互いに素であることに反する.\\
よって,$\sqrt{2}$は無理数である.
\newpage
\section{同値変形4（存在条件の代入原理）}
この章は,のちに扱う\ans{軌跡・領域・通過領域}で必須の事項になります.いつも以上に気合いをいれていきましょう.
\begin{itembox}
    [l]{\enpitu 存在条件の代入原理}
    $Q(x)$を$x$の条件とする.このとき,
    \begin{align*}
        \exists x,\begin{cases}
            x=\alpha\\
            Q(x)
        \end{cases}\doti Q(\alpha)
    \end{align*}
\end{itembox}\\
ex)\\
\kakkoichi$\exists x,\begin{cases}
    x=3\\
    xt+6=0
\end{cases}\doti3t+6=0\doti t=-2$\\\\
\kakkoni$\exists x,\begin{cases}
    ax=2\\
    3x=b
\end{cases}\doti a\cdot\frac{b}{3}=2\doti ab=6$\\
\begin{itembox}
    [l]{\enpitu 量化記号と$\vee,\wedge$の関係性}
    \kakkoichi$\exists x,\p{P(x)\wedge Q(x)}\marubatu \p{\exists x,P(x)}\wedge\p{\exists x,Q(x)}$\\\\
    \kakkoni$\exists x,\p{P(x)\vee Q(x)}\marumaru\p{\exists x,P(x)}\vee\p{\exists x,Q(x)}$\\\\
    \kakkosan$\forall x,\p{P(x)\wedge Q(x)}\marumaru\p{\forall x,P(x)}\wedge\p{\forall x,Q(x)}$\\\\
    \kakkoshi$\forall x,\p{P(x)\vee Q(x)}\batumaru\p{\forall x,P(x)}\vee\p{\forall x,Q(x)}$
\end{itembox}
\newpage
\noindent\ba{15$\bullet$1}\\
次の各条件が成立するための必要十分条件をできるだけ簡単な形で求めよ.文字はすべて実数である.\\
\kakkoichi $\exists x,\begin{cases}
    |x|<a\\
    x+b=2
\end{cases}$\\
\kakkoni$\exists x,\begin{cases}
    ...
    bx=c
\end{cases}$\\
\kakkosan$\exists x,\begin{cases}
    ax=a^2\\
    bx=a+1
\end{cases}$\\
\kakkoshi$\exists x,\begin{cases}
    x^2-kx-1=0\\
    x^3-k^2x=0
\end{cases}$\\
\kakkogo$\exists x,\begin{cases}
    x^2+ax+b=0\\
    x^2+bx+a=0
\end{cases}$
\newpage
\begin{center}
    \ans{15章の問題演習}
\end{center}
\reibanichi チェックテストの(31)$\sim$(36)を確認せよ.
\newpage
\section{同値変形5（複数文字の存在条件の代入原理）}
同値変形4でやった1文字の存在条件の代入原理とさほど変わりませんが,必要十分になるための条件が増えることが多いです.
\begin{itembox}
    [l]{\enpitu ド・モルガンの法則}
    大人向けのやつです.
    \begin{align*}
        &\kakkoichi\neg［\exists x,P(x)］\doti\forall x,\neg P(x)\\
        &\kakkoni\neg［\forall x,P(x)］\doti\forall x,P(x)
    \end{align*}
\end{itembox}
\newpage
\noindent\ba{16$\bullet$2}\\
次の各条件が成り立つための必要十分条件をできるだけ簡単な形で求めよ.\\
\kakkoichi$\exists x,\begin{cases}
    x^2-kx+1=0\\
    x^3-(k+1)x^2-k+4=0
\end{cases}$\\\\
\kakkoni$\exists\tvec<0,1>[x,y],\begin{cases}
    x^2+y^2=1\\
    y=x+a
\end{cases}$\\\\
\kakkosan$\exists\tvec<0,1>[x,y],\begin{cases}
    x+y=2t\\
    xy=t+2
\end{cases}$\\\\
\kakkoshi$\exists(x,y),\begin{cases}
    x^2-y^2=1\\
    y=x^2+k
\end{cases}$\\
\kakkogo$\exists(x,y),\begin{cases}
    x^2-3y^2+6x+9=0\\
    x^2+y^2-2ax+a^2-1=0
\end{cases}（東京大）$\\
\ba{16$\bullet$2}\\
実数$b$をどのように定めても,$-1\leq x\leq1$の範囲の適当な$x$をとることによって,$|ax+b|\geq1$が成立するための必要十分条件を求めよ.
\newpage
\section{写像1（写像とは）}
まずは,ウォーミングアップから.\\
\ba{17$\bullet$1}\\
グラウンドに1000人の生徒がいます.この中から背が一番高い人を探したいです.どのようにして,探すか思いつく限りアイデアを出してください.
\newpage
\begin{itembox}
    [l]{\enpitu 写像の定義$\maruichi$（ちょっと雑）}
    $A,B$を空でない集合とし,$a\in A,b\in B$であるとする.\\
    $Aの元aに対し,Bの元bが$唯一つ対応するようなルール$f$があるとき,この$f$を写像といい,
    \begin{center}
        $f：A\to B$や$A\xrightarrow{f}B$
    \end{center}
    と書く.
\end{itembox}
\theme{イメージ}
\vspace{5cm}
\begin{itembox}
    [l]{\enpitu 写像の定義$\maruni$（きちんとした）}
    空でない集合$A,B$に対し,それぞれの元を$a,b$とする.\\
    このとき,写像$\Gamma$とは,
    \begin{center}
    $\begin{array}{rccc}
    \Gamma\colon &A                     &\longrightarrow& B                     \\
        & \rotatebox{90}{$\in$}&               & \rotatebox{90}{$\in$} \\
        & a                    & \longmapsto   & b
\end{array}$
    \end{center}
    を満たすような,三つ組$\p{\Gamma,A,B}$のことである.\\
    この$A$のことを\ans{定義域}といい,$B$のことを\ans{終域,値域}などという.\\
    このとき,$\Gamma$は単射であることに注意せよ.
\end{itembox}
\newpage
\begin{itembox}
    [l]{\enpitu 順像法と逆像法}
    三つ組$\p{\Gamma,A,B}$に対して,$\Gamma$による$A$の像（値域）$W=\Gamma\p{A}$を求める方法として,次の2つがある.\\
        $\bullet 順像法：Aの各元aの\Gamma による像\Gamma(a)を\ans{すべて集める}ことにより,Wを求めようとする考え方.\\
        \bullet 逆像法：Bの各元bに対して,\ans{b\in Wを満たすb}\\すなわち,\ans{\Gamma(a)=bを満たすaがA内に存在するようなb}$を
        \ans{すべて集める}ことによって$W$を求めようとする考え方.
\end{itembox}\\
\theme{イメージ}
\newpage
\noindent\ba{17$\bullet$2}\\
次の各問に答えよ.\\
\kakkoichi $x$が全実数を動くとき,2次関数$y=f(x)=x^2-4x+1$の値域を$W_1$とする.$W_1$を求めよ.\\
\kakkoni $xがx\geq2$を動くとき,関数$f(x)=\frac{x}{x+1}$の値域を$W_2$とする.$W_2$を求めよ.\\\\
\ba{17$\bullet$3}\\
$x^2\leq y\leq x+2$のとき,$z=x-2y$の値域$W$を求めよ.
\newpage
\begin{center}
    \ans{17章の問題演習}
\end{center}
\reibanichi$\theta$が全実数を動くとき,関数$f(\theta)=\cos\theta+\sin\theta$の値域$W$を求めよ.\\\\
\reibanni$0\leq x\leq2$のとき,$y=\frac{x^2+3}{x+1}$の値域$W$を求めよ.\\\\
\reibansan$0\leq x\leq y\leq3$で表される$xy$平面上の領域$D$を$(x,y)$が動くとき,\\2変数関数$z=f(x,y)=x^2-4x+y$の値域$W$を求めよ.
\newpage
\section{写像2}
\noindent\ba{18$\bullet$1}\\
連立不等式
\begin{align*}
    \begin{cases}
    y\geq x^2-2x+1\\
...
\end{align*}
の表す領域を$D$とする.\\
\kakkoichi $D$を座標平面上に図示せよ.\\
\kakkoni $a$を実数とする.点$(x,y)$が$D$を動くとき,$-ax+y$の最大値を$f(a)$とする.$f(a)$を求めよ.\\\\
\ba{18$\bullet$2}\\
$x^2-2xy+2y^2=4$のとき,\\
\kakkoichi$x$のとり得る値の範囲を求めよ.\\
\kakkoni$x+y$のとり得る値の範囲を求めよ.\\\\
\ba{18$\bullet$3}\\
$f(x,y)=3y^2-4xy+2x-2y+1$とする.$x,y$が$x\geq0,y\geq0,3x+y\leq1$を満たして動くとき,$f(x,y)$の最小値$m$を求めよ.
\newpage
\begin{center}
    \ans{18章の問題演習}
\end{center}
\noindent\reibanichi$-\sqrt{3}\leq x\leq\sqrt{3}y,0<y\leq2$における$z=x^3-3xy^2+y^4$の最大値$M$および最小値$m$を
求めよ.\\\\
\reibanni 座標平面上において,点$(x,y)$が方程式$x^2+y^2=1$で表される円の上を動くとき,
\begin{align*}
    z=\dfrac{2x+y+1}{3x+y+5}
\end{align*}
の最大値と最小値を求めよ.
\newpage
\section{写像3}
...
次の各問いに答えよ.\\
\kakkoichi$実数t$が正の実数全体を動くとき,$\tvec<0,1>[x,y]=\tvec<0,1>[t+1,t^2-1]$で定まる点$（x,y）$の軌跡$W$を求めよ.\\\\
\kakkoni 実数$\theta$が実数全体を動くとき,$\tvec<0,1>[x,y]=\tvec<0,1>[2\cos\theta+3,2\sin\theta-1]$で定まる点$（x,y）$の軌跡$W$を求めよ.\\\\
\ba{19$\bullet$2}\\
実数$k$が全実数を動くとき,$xy$平面上の2つの直線
\begin{center}
    $\ell:(k+1)x+(1-k)y+k-1=0,　m:kx+y+1=0$
\end{center}
の交点の軌跡を求めよ.\\\\
\ba{19$\bullet$3}\\
$t$が$-1$以上の実数を動くとき,
\begin{center}
    $x=\frac{1}{t^2+1},　y=\frac{t}{t^2+1}$
\end{center}
で定まるP$(x,y)$の軌跡を$C$とする.$Cの式をx,yで表し,曲線Cを図示せよ.$
\newpage
\begin{center}
    \ans{19章の問題演習}
\end{center}
\noindent\reibanichi$\theta$が$0\leq\theta\leq\pi$を動くとき,点P$\p{\cos\theta+2\sin\theta,-2\cos\theta+\sin\theta}$の軌跡$W$を求めよ.\\\\
\reibanni 曲線$xy=1$上に2点$\mathrm{A}(1,1),\mathrm{B}(-1,-1)$をとる.直線$y=x$に関する点Rの対称点を$\mathrm{R}^\prime$とする.
Rがこの曲線上（ただし,A,Bを除く）を動くとき,直線RAと直線$\mathrm{R}^\prime B$との交点Pの軌跡を求めよ.
\newpage
\section{写像4}
\noindent\ba{20$\bullet$1}\\
実数$x,y$が$\begin{cases}
    0\leq x\leq1\\
    0\leq y\leq1
\end{cases}$を満たして動くとき,$\begin{cases}
    X=2x+y\\
    Y=x-y
\end{cases}$
で定まる点$(X,Y)$の存在し得る範囲$W$を求めよ.\\\\
\ba{20$\bullet$2}\\
平面上で,点Pが1辺の長さ2の正三角形の周上を動き,点Qは半径1の円周上を動く.このとき,PQの中点Mの動く範囲の面積$S$を求めよ.\\\\
\ba{20$\bullet$3}\\
放物線$y=x^2$のうち,$-1\leq x\leq1$を満たす部分を$C$とする.座標平面上の原点Oと点A$(1,0)$を考える.\\
\kakkoichi 点Pが$C$上を動くとき,
\begin{align*}
    \Vec{OQ}=2\Vec{OP}
\end{align*}
を満たす点Qの軌跡を求めよ.\\
\kakkoni 点Pが$C$上を動き,点Rが線分OA上を動くとき,
\begin{align*}
    \Vec{OS}=2\Vec{OP}+\Vec{OR}
\end{align*}
を満たす点Sが動く領域を座標平面上に図示し,その面積を求めよ.\\
\hspace{10cm}（東京大：\ans{文理共通問題}）
\newpage
\section{写像5}
\noindent\ba{21$\bullet$1}\\
\kakkoichi 点P$(x,y)$が平面全体を動くとき,\\$X=x+y,Y=xy$で定まる点Q$(X,Y)$の描く図形$W$を図示せよ.\\
\kakkoni 点P$(x,y)$が$0\leq x\leq1,0\leq y\leq1$を満たして動くとき,\\
$X=x+y,Y=xy$で定まる点Q$(X,Y)$の描く軌跡$W$を図示せよ.\\\\
\ba{21$\bullet$2}\\
実数$x,y$が$x^2+xy+y^2=1$を満たして動くとき,次の問に答えよ.\\
\kakkoichi $x+y=u,xy=v$とする.点$(u,v)$の存在範囲を$uv$平面に図示せよ.\\
\kakkoni $x^2+y^2$の最大値と最小値を求めよ.\\
\hspace{10cm}（同志社大）
\newpage
\section{写像6}
通過領域とは,次のようなイメージを持つとよいと思います.\\
\theme{イメージ}
\newpage

\noindent\ba{22$\bullet$1}\\
$xy$平面上の直線$l_t：y=tx+t^2$を考える.\\
$t$が実数全体を動くとき,$l_t$が通過する範囲$W$を求め,図示せよ.\\\\
\ba{22$\bullet$2}\\
$xy$平面上の円
\begin{center}
    $C_a：(x-a)^2+(y-a)^2=a^2+1$
\end{center}
について,$a$が0以上のすべての実数を動くとき,$C_a$が動く範囲を図示せよ.\\
\hspace{10cm}（北海道大）\\\\
\ba{22$\bullet$3}\\
$0\leq t\leq1$を満たす実数$t$に対して,$xy$平面上の点A,Bを
\begin{align*}
    \mathrm{A}\p{\dfrac{2(t^2+t+1)}{t^2+1},-2},\mathrm{B}\p{\dfrac{2}{3}t,-2t}
\end{align*}
と定める.$t$が$0\leq t\leq1$を動くとき,直線ABの通り得る範囲を図示せよ.\\
\hspace{10cm}（東京大）
\end{document}